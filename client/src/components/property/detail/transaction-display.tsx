...
export default function TransactionDisplay({ transaction }: TransactionDisplayProps) {
    ...
    return (
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-2 py-4 items-center border-b border-b-neutral-300">
            <div>
                <header>
                    <span class="lg:hidden font-bold">Concepto: </span>
                    <span>{Concept.length === 0 ? "Sin Concepto" : Concept}</span>
                </header>
            </div>
            <div>
                <header>
                    <span class="lg:hidden font-bold">Fecha: </span>
                    <span>{Date.split("T")[0]}</span>
                </header>
            </div>
            <div>
                <header>
                    <span class="lg:hidden font-bold">Dolares: </span>
                    <span>{Dollars} $</span>
                </header>
            </div>
            <div>
                <header>
                    <span class="lg:hidden font-bold">Tipo: </span>
